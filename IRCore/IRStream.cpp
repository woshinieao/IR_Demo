...
UINT16 LineAck(UINT16 Linenum, UINT32 Framenum, long lParameter)
{
    ...
    {
        command[4] = i+1;
        pIRStream->Send((char*)(&(command[0])), 14);
    }
    return 0;
}
...
long ConfigReceive(char recvbuffer[], long lParameter)
{
    CIRStream* pIRStream = (CIRStream*)lParameter;
    if (pIRStream->ConfigCallback)
    {
        pIRStream->ConfigCallback((long)recvbuffer, pIRStream->parameter);
    }
    return 1;
}
...
{
    return Init(port, NULL, cbf, NULL, NULL, param);
}
/*
...
bool CIRStream::Init(int port, char* ip, CBF_IR cbf_stm, CBF_IR cbf_cmd, CBF_IR cbf_cfg, long param)
{
    if (port)
    {
        pIRConnect = new CIRConnect;
    ...
    if (ip)
    {
        ...
        pIRConfig = new CIRConnect;
        pIRConfig->Bind(10002, ip, ConfigReceive, (long)this);
        ConfigCallback = cbf_cfg;
    }
    parameter = param;
    return true;
}
...
//        {'\xaa', '\xaa', '\xaa', '\xaa', '\x00', '\x00', '\x03', '\xe8', '\x00', '\x04', '\x00', '\x00', '\x00', '\x01'},//数字视频开
//        {'\xaa', '\xaa', '\xaa', '\xaa', '\x00', '\x00', '\x03', '\xe8', '\x00', '\x04', '\x00', '\x00', '\x00', '\x00'},//数字视频关
//        {'\xaa', '\xaa', '\xaa', '\xaa', '\x00', '\x00', '\x00', '\x49', '\x00', '\x04','\x00', '\x00','\x00', '\x01'}, //快门
//        {'\xaa', '\xaa', '\xaa', '\xaa', '\x00', '\x00', '\x00', '\xD1', '\x00', '\x04', '\x43', '\x4f', '\x4e', '\x4e'}//链接命令
//    };

//    return Command(COMMAND[command], 14);
//}

bool CIRStream::Command( int cmd, int param)
{
    char command[14] = {'\xaa', '\xaa', '\xaa', '\xaa', '\x00', '\x00', '\x00', '\x00', '\x00', '\x04', '\x00', '\x00', '\x00', '\x01'};

    command[4] = (cmd >> 24) & 0xff;
    command[5] = (cmd >> 16) & 0xff;
    command[6] = (cmd >> 8) & 0xff;
    command[7] = cmd & 0xff;

    command[10] = (param >> 24) & 0xff;
    command[11] = (param >> 16) & 0xff;
    command[12] = (param >> 8) & 0xff;
    command[13] = param & 0xff;

    return pIRCommand ? pIRCommand->Send(command, 14) : false;
}

bool CIRStream::Send( char* pBuff, int length)
{
    return pIRCommand ? pIRCommand->Send(pBuff, length) : false;
}

bool CIRStream::Config( char* command, int length)
{
    return pIRConfig ? pIRConfig->Send(command, length) : false;
}
